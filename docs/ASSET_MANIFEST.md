...
`--check` verifies, in order: the manifest parses and its enums are valid; every tracked media file is listed exactly once (no more, no less); every listed file's computed facts (bytes/SHA-256/raster dimensions) match what's actually on disk; every raster file's **container is actually complete AND carries real encoded payload** (not just its header/dimensions — a PNG/JPEG/GIF/WebP with a valid, readable header and a valid-looking end marker but literally no pixel data — e.g. a PNG with only `IHDR`+`IEND` and zero `IDAT` chunks — is flagged as incomplete; PNG payload is verified by zlib-inflating the concatenated `IDAT` stream and, for non-interlaced images, matching the exact byte count IHDR predicts); every committed runtime binding resolves to an asset classified `scope: "runtime"`; every binding's `fallbackKind` is a recognized value, and every `asset-chain` fallback names real binding keys that terminate in a required binding or a non-chain fallback rather than dead-ending; every required runtime binding has a committed file; every optional runtime binding declares a fallback; and the whole manifest is byte-for-byte canonical (so a hand-edited or stale file fails even if every individual fact happens to still be correct).
...
`tools/asset-manifest-test.mjs` (wired into `npm test`) does more than validate the JSON — it **boots the real game in a headless browser and reads its actual `SPRITES` registry, `HERO_IDENTITIES`, DOM `<img>` sources, computed CSS, the Character screen's rendered paper-doll `<img>` elements, and `bgMusic.src`**, then cross-checks those live values against **every** declared `runtimeBinding` — committed or not — by exact key AND exact path, not just "this loads." Concretely: `title_bg`'s declared path is compared against the actual computed `background-image` URL (not merely whether that URL loads); `HERO_IDENTITIES.titlePortrait`/`paperDollDirection` are read directly and compared against the corresponding bindings; and the Character screen is actually opened (with synthetic gear equipped) to read the real rendered paper-doll `<img src>` values for the base layer and all four equipment-overlay layers, rather than inferring correctness from "this path happens to also appear under a different SPRITES key." This is what catches drift a hand-maintained table can't: if a future code change adds a new `loadSprite()` call, removes one, or changes a path template, the live cross-check fails even though the manifest JSON itself is internally consistent. (This is exactly how the `equipment_attack_*_cape` binding was discovered during this PR's own development — the live registry had it, the first draft of the declarative table didn't.) The four never-registered NPC/building bindings (`forge_building`/`npc_bram`/`npc_gunnar`/`npc_dumpling_vendor`) are verified against a live parse of `js/02-data-state.js`'s actual `loadSprite()` call sites, not a hard-coded exception list — so if a future change adds their missing registration, the test's assumption is checked against real source rather than silently going stale. The same suite also attaches a real `requestfailed` listener (via `smoke-test.mjs`'s `launch(urlSuffix, { onPage })` hook) to observe actual failed asset requests at the network level, rather than relying on console-error text that the shared test launcher deliberately filters for expected misses.
