...
- All five areas (Farm, Town, Wilds, Deep Woods, Mine) default to the isometric renderer since the 2026-07-30 eight-direction slice; `?iso=0` remains the top-down escape hatch.
...
- **Superseded 2026-07-30 by owner call:** the four-facing compatibility subset is no longer the runtime default. The engine consumes all eight facings per hero (right=SE, down=SW, left=NW, up=NE, down-right=S, down-left=W, up-left=N, up-right=E), with walk strips per facing sourced from the owner's manual web-Creator regeneration of both heroes, curated to the engine's `{stand, step A, stand, step B}` contract, and played by the iso renderer.
