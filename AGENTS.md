...
- For asset generation or integration, read `tools/pipeline/PIPELINE.md`, `tools/pipeline/PIXELLAB_API.md`, `tools/pipeline/PIXELLAB_MCP.md`, and `.claude/skills/asset-generation/SKILL.md`. **Check `PIXELLAB_API.md` before any generation call** — it holds the cost model and the output-size rule that decide whether a sprite is billed at 1 generation or 8. Treat PixelLab's linked live reference as the current tool authority; never commit tokens or a vendor-documentation snapshot. `tools/3D_ISO_SPRITE_PIPELINE.md` is historical and must not be restarted.
