...
// --- Save integrity: rejection, migration, canonical storage, round-trip ---
{
  ...
  const r = await page.evaluate(async () => {
    ...
    // every enemy alive.
    var v0 = { gold: 33, seeds: 3, crops: 2, tiles: { '5,5': { status: 'growing', plantedAt: 1 } } };
    var v1 = { gold: 44, level: 4, farmTiles: { '6,6': { status: 'ready', plantedAt: 1, type: 'corn' } },
               townTiles: { '7,7': { status: 'empty' } }, gear: { weapon: 'wooden_sword' },
    ...
    var v2 = { version: 2, area: 'wilds', x: 96, y: 128,
      player: { gold: 55, level: 7, xp: 10, gear: { weapon: 'steel_sword', head: 'crystal_crown' },
                ...
                dumplingDough: 12 },
      areas: { farm: { tiles: { '5,5': { status: 'growing', plantedAt: 2, type: 'carrot' } } } } };
    function m(s) { return ingestSaveObject(s); }
    var r0 = m(v0), r1 = m(v1), r2 = m(v2);
    ...
    out.v0 = r0.ok && r0.state.version === 3 && r0.state.player.gold === 33 &&
      ...
      r0.state.areas.farm.tiles['5,5'].status === 'growing' && allAlive(r0.state);
    out.v1 = r1.ok && r1.state.player.gold === 44 && r1.state.player.level === 4 &&
      r1.state.player.gear.weapon === 'wooden_sword' &&
      r1.state.areas.farm.tiles['6,6'].type === 'corn' &&
      r1.state.areas.town.tiles['7,7'].status === 'empty' &&
      r1.state.player.killQuest && r1.state.player.killQuest.progress === 1 &&
      r1.state.player.killQuest.count === 3 && allAlive(r1.state);
  ...
  check('SAVE: v1 fixture migrates with gear, soil, and active quest intact', r.v1);
...
// --- ELD-STATE-001: enemy life/respawn state is owned by the selected profile ---
{
  ...
  const r = await page.evaluate(() => {
    ...
    openCombat(bat);
    combatEnemy.hp = 0;
    winCombat();                       // bat dead with a timer
    openCombat(currentEnemies[2]);
    loseCombat();                      // faint: current area all alive, timers cleared
    out.lossRevivesArea = currentEnemies.every(function (e) { return e.alive && e.respawnAt === 0; });
    switchProfile();
    selectProfile('mage');
    activateArea('wilds');
    out.lossLeftMageAlone = currentEnemies.every(function (e) { return e.alive; });
    switchProfile();
    localStorage.clear();
    return out;
  });
  ...
  check('ELD-STATE-001: combat loss leaves the other profile untouched', r.lossLeftMageAlone);
  await browser.close();
}
