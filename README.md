...
tests), `npm run assets:verify` (regenerates every production asset in memory and fails unless it
...
and tracked asset outputs are only ever rewritten by an explicit `npm run assets:build`. CI uploads
